import { AlertProvider } from '../interfaces';
import { MarineAlert } from '../types';

interface IMDMarineWarningResponse {
  status: string;
  data: Array<{
    id: string;
    warningType: string;
 ...
 */
export class IMDProvider implements AlertProvider {
  name = 'imd_api';
  ...
  async fetchAlerts(lat: number, lon: number): Promise<MarineAlert[]> {
    ...
    const url =
      `https://api.imd.gov.in/api/v1/marine/warnings` +
    ...
    if (!response.ok) {
      throw new Error(
        `IMD API error: ${response.status} ${response.statusText}`
      );
    }

    const body = (await response.json()) as IMDMarineWarningResponse;
  ...
  private normalizeSeverity(raw: string): 'INFO' | 'WARNING' | 'CRITICAL' {
    const s = (raw ?? '').toUpperCase();
    if (s.includes('RED') || s.includes('EXTREME') || s.includes('SEVERE')) return 'CRITICAL';
    if (s.includes('ORANGE') || s.includes('WARNING')) return 'WARNING';
    return 'INFO';
  }
}
